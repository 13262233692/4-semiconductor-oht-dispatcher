...
export class DistributedLockService implements OnModuleInit {
  ...
  private readonly LOCK_KEY_PREFIX = 'grid:lock:';
  private readonly DEFAULT_LOCK_TTL_MS = 15000;
  private readonly MAX_RETRY_ATTEMPTS = 10;
  private readonly RETRY_DELAY_BASE_MS = 20;
  ...
  private activeLocks: Map<string, { key: string; owner: string; expiresAt: number; ttl: number; timer?: ReturnType<typeof setTimeout> }> = new Map();
  ...
  private getLockKey(gridX: number, gridY: number): string {
    return `${this.LOCK_KEY_PREFIX}${gridX}:${gridY}`;
  }

  async acquireGridLock(
    gridX: number,
    gridY: number,
    ohtId: string,
  ...
  async acquirePathLocks(
    segments: Array<{ toX: number; toY: number }>,
    ohtId: string,
    ttlMs: number = this.DEFAULT_LOCK_TTL_MS,
  ): Promise<{ success: boolean; acquired: string[]; error?: string }> {
  ...
  async releaseGridLock(gridX: number, gridY: number, ohtId: string): Promise<boolean> {
    const key = this.getLockKey(gridX, gridY);
  ...
  async renewGridLock(gridX: number, gridY: number, ohtId: string, ttlMs: number = this.DEFAULT_LOCK_TTL_MS): Promise<boolean> {
    const key = this.getLockKey(gridX, gridY);

    try {
      const now = Date.now();
      const result = await this.redis.eval(
        LUA_SCRIPTS.RENEW_LOCK,
        [key],
        [ohtId, ttlMs, now],
      );
  ...
  async getLockInfo(gridX: number, gridY: number): Promise<LockInfo | null> {
    const key = this.getLockKey(gridX, gridY);

    try {
      const result = await this.redis.eval(
        LUA_SCRIPTS.GET_LOCK_INFO,
        [key],
        [],
      );
  ...
  async forceClearLock(gridX: number, gridY: number): Promise<boolean> {
    const key = this.getLockKey(gridX, gridY);

    try {
      const result = await this.redis.eval(
        LUA_SCRIPTS.FORCE_CLEAR_LOCK,
        [key],
        [],
      );
  ...
  async scanAndClearZombieLocks() {
    ...
    try {
      ...
      do {
        const result = await this.redis.eval(
          LUA_SCRIPTS.SCAN_ZOMBIE_LOCKS,
          [],
          [cursor, this.LOCK_KEY_PREFIX, now, limit],
        );
        ...
        for (let i = 0; i < entries.length; i += 2) {
          ...
          const value = entries[i + 1] as string;
          totalDetected++;
          ...
          try {
            ...
            const parts = value.split(':');
            this.logger.warn(
              `Cleared zombie lock ${key} (owner=${parts[0]}, expired=${new Date(Number(parts[1])).toISOString()})`,
            );
          } catch (e) {
  ...
  private trackLock(key: string, owner: string, expiresAt: number, ttl: number) {
  ...
  private untrackLock(key: string) {
    const existing = this.activeLocks.get(key);
    if (existing?.timer) {
      clearTimeout(existing.timer);
    }
  ...
  private updateLockExpiry(key: string, expiresAt: number, ttl: number) {
  ...
  getActiveLocks(): Array<{ key: string; owner: string; expiresInMs: number }> {
    const now = Date.now();
